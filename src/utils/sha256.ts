import { utf8ToBytes } from "./utf8-bytes";
...
export function sha256Bytes(data: Uint8Array): Uint8Array {
  ...
  const padLength = ((56 - ((data.length + 1) % 64)) + 64) % 64;
  ...
  for (let offset = 0; offset < totalLength; offset += 64) {
    sha256Block(state, buffer, offset);
  }
...
export function sha256Hex(value: string): string {
  return bytesToHex(sha256Bytes(utf8ToBytes(value)));
}
...
export function hmacSha256Hex(key: Uint8Array, message: string): string {
  ...
  const inner = sha256Bytes(concatBytes(iKeyPad, utf8ToBytes(message)));
  return bytesToHex(sha256Bytes(concatBytes(oKeyPad, inner)));
}
